...
@routes.get(path='/users/{id}/cart')
async def cart(request: Request) -> Response:
    try:
        user_id = int(request.match_info['id'])
        pool: asyncpg.Pool = request.app['db']
        result: list[asyncpg.Record] = await pool.fetch(query=f'SELECT product_id FROM user_cart WHERE user_id = {user_id}')
...
app = Application()
app.on_startup.append(functools.partial(create_db_pool, host='127.0.0.1', port=5432, user='postgres', password=decouple.config('DB_PASSWORD'), database='cart'))
app.on_cleanup.append(destroy_db_pool)
app.add_routes(routes=routes)
run_app(app=app, host='127.0.0.1', port=8003)